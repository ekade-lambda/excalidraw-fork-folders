...
describe("Board System :: Phase 11 Delete Orchestration", () => {
  ...
  const mockAPI = () => {
    ...
    return {
      ...
      __setElements: (els: any[]) => { elements = els; }
    } as any;
  };
  ...
  it("1. Delete de carpeta vacia y representacion visual local", async () => {
    ...
    await repo.saveBoard({ schemaVersion: 1, boardId: (r1 as any).boardId, elements: [], files: {}, name: "A", viewport: null, updatedAt: 1 });

    const api = mockAPI();
    api.__setElements([
      { id: "el1", type: "rectangle", customData: { type: "board-folder", folderId: (r1 as any).folderId } },
      { id: "el2", type: "text", customData: { type: "board-folder-text", folderId: (r1 as any).folderId } },
      { id: "el3", type: "rectangle" } // Otro elemento
    ]);

    const res = await deleteFolder({ repo, excalidrawAPI: api, folderId: (r1 as any).folderId });
  ...
  it("2. Delete de jerarquia profunda persistida", async () => {
    ...
    const api = mockAPI();
    await deleteFolder({ repo, excalidrawAPI: api, folderId: (r1 as any).folderId });
  ...
  it("4. Zombie Rendering (Undo visual) es manejado pacíficamente por openFolderInternal", async () => {
    ...
    const res = await openFolder({ repo, excalidrawAPI: mockAPI(), folderId: "deleted-id" });
