...
import {
  syncStructuralElements,
  isStructuralElement,
} from "../../boards/host/reconciliation";
...
describe("reconciliation :: syncStructuralElements (10C fixes)", () => {
  it("1. isStructuralElement identifica correctamente mediante customData", () => {
    ...
    const el2 = {
      id: "2",
      customData: { folderBoard: { kind: "folder", role: "image" } },
  ...
  it("2. Folder ausente en canvas + presente en Graph + presente físicamente -> se inyectan", () => {
    ...
    const { primary, text } = buildFolderVisual({
      folderId: fId,
      boardId: "board-f1" as BoardId,
      name: "Folder 1",
      sceneX: 10,
    ...
    const { elements: nextElements, didChange } = syncStructuralElements(
      graph,
      currentElements,
      rootId,
      remoteElements,
  ...
  it("4. Canvas con trazos + folder remoto nuevo -> trazos permanecen intactos", () => {
    ...
    const { primary } = buildFolderVisual({
      folderId: fId,
      boardId: "board-f1" as BoardId,
      name: "Folder 1",
      sceneX: 10,
  ...
  it("5. Carpeta existente en ambas -> NO se modifica su posición (LWW activo)", () => {
    ...
    const myVisuals = buildFolderVisual({
      folderId: fId,
      boardId: "board-f1" as BoardId,
      name: "Folder 1",
      sceneX: 500,
    ...
    const remoteVisuals = buildFolderVisual({
      folderId: fId,
      boardId: "board-f1" as BoardId,
      name: "Folder 1",
      sceneX: 10,
    ...
    const resultingPrimary = nextElements.find(
      (e) => e.id === myVisuals.primary.id,
    );
