...
path = "excalidraw-app/tests/boards/gc.test.ts"
...
describe("Board System :: Phase 11 GC Activation", () => {
  ...
  it("1. GC no borra payload referenciado", async () => {
    ...
    await repo.saveBoard({ schemaVersion: 1, boardId, elements: [], files: {}, name: "A", viewport: null, updatedAt: 1 });
  ...
  it("2. Payload huérfano protegido por WAR NO es recolectado", async () => {
    ...
    const mockAPI = () => ({ getSceneElementsIncludingDeleted: () => [], updateScene: () => {} } as any);
    await deleteFolder({ repo, excalidrawAPI: mockAPI(), folderId: (r1 as any).folderId });

    // Inyectamos WAR activo
    window.localStorage.setItem(`${STORAGE_KEYS.BOARDS_WAR_PREFIX}${boardId}`, Date.now().toString());
  ...
  it("3. Payload huérfano sin WAR es recolectado", async () => {
    ...
    window.localStorage.setItem(`${STORAGE_KEYS.BOARDS_WAR_PREFIX}${boardId}`, (Date.now() - 2 * 60 * 60 * 1000).toString());

    const nextGraph = await repo.load();
    await repo.runGarbageCollector(nextGraph!);

    const savedAfter = window.localStorage.getItem(`${STORAGE_KEYS.BOARDS_BOARD_PREFIX}${boardId}`);
