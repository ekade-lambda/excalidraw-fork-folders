report = """# FASE 10B — Auditoría Final de Consistencia y Concurrencia
...
- **`boardService.ts` (`saveCurrentBoard`):** Se ejecuta en cada autosave. Llama *únicamente* a `repo.saveBoard(data)`. **Nunca** llama a `save(graph)`.
...
1. El repositorio expondrá una API transaccional explícita:
...
3. **La excepción (`duplicate.ts`):** Como está prohibido modificarlo, el repositorio interceptará *exclusivamente* `clonePhysicalBoardsSync` (que lógicamente siempre crea tableros nuevos) para añadir los IDs al WAR. Y el repositorio interceptará `saveSync(graph)` para limpiar **solamente los IDs síncronos** que haya inyectado `clonePhysicalBoardsSync`. Esto sella la brecha de `duplicate.ts` sin tocar su código.
...
Tienes razón. `LocalStorage` no es atómico para operaciones R-M-W. Un solo objeto `active-writes` sufriría de *lost updates*.
**Solución Diseñada:**
Usaremos **entradas individuales** con prefijo: `excalidraw-war-<boardId>`.
- Tab A escribe `localStorage.setItem("excalidraw-war-X", timestamp)`.
- Tab B escribe `localStorage.setItem("excalidraw-war-Y", timestamp)`.
Es 100% atómico y libre de colisiones a nivel de sistema de archivos (LS).
El GC iterará sobre las llaves buscando el prefijo `excalidraw-war-`.
...
*Regla de Oro:* El GC **nunca** elimina un componente físico (ni puntero ni payload) si el `boardId` existe en el Grafo o en el WAR. Una inconsistencia interna (Puntero sin IDB) no autoriza al GC a borrar el puntero si está referenciado.
...
with open("reporte_auditoria_fase_10b.md", "w", encoding="utf-8") as f:
    f.write(report)
print("Generado")
