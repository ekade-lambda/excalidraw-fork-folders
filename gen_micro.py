...
path = "microauditoria_post_fase_10c.md"
content = """# MICROAUDITORÍA PARA CORRECCIONES DE FASE 10C
...
3. **T2 (Carrera):** El usuario dibuja, el timer dispara `saveCurrentBoard()`. Excalidraw lee la escena *sin la carpeta remota* y la guarda en disco.
...
### Solución Propuesta
Aumentar la granularidad de la identidad utilizando el `role` ya presente en la metadata (`meta.role`).
Las claves serán: `folder:${meta.folderId}:${meta.role}` (ej. `folder:f1:image`, `folder:f1:text`).
**Efecto:**
