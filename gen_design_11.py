...
path = "auditoria_diseño_fase_11.md"
content = """# DISEÑO DETALLADO — FASE 11 (CICLO DE VIDA)
...
- La eliminación estructural *sólo* es posible explícitamente vía Menú Contextual del Board System -> "Delete".
...
- Al deshacer (`Ctrl+Z`) después de un Delete Estructural, el folder *resucitará visualmente*. 
...
El `runGarbageCollector` contenido en `GarbageCollector.ts` se activará como un *Deferred Job* (Fire-and-Forget) poco intrusivo.
...
- Se ejecutará *solo una vez por sesión de pestaña*, limpiando los payloads huérfanos que hayan superado el WAR (1 hora de TTL).
