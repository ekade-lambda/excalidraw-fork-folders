report = """# FASE 10: Informe de Diagnóstico Forense del Modelo de Datos
...
El objeto `f` es extraído de `Object.values(boardData.files)`. Su tipo es `BinaryFileData` (definido en Core: `packages/excalidraw/types.ts`).
Tiene exactamente estas propiedades:
- `id`: FileId
...
**Ninguno para datos nuevos**, asumiendo que un "Hard Refresh" del navegador sincronice el bundle.
**Para normalizar datos antiguos**: Dentro de `loadBoardIntoEditor`, justo antes de `excalidrawAPI.addFiles(files)`: iterar los archivos `f` y, si `f.dataURL` empieza por `data:image/svg+xml;charset=utf-8`, transformarlo al vuelo convirtiendo el URI-encoded string a Base64 y actualizar la propiedad `f.dataURL` antes de entregarlo al Core.
"""

with open("reporte_final_investigacion.md", "w", encoding="utf-8") as f:
    f.write(report)
