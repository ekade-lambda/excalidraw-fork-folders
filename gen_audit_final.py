report = """# FASE 10 — Arquitectura Definitiva (Tercera Auditoría)
...
- *Mitigación de Crashes:* Solo si la pestaña crashea irremediablemente, el ID quedará en `active-writes`. Aquí, y solo como mecanismo de recuperación ante desastres (no para concurrencia), el GC aplicará un TTL de 1 hora a los registros colgados en `active-writes` para liberarlos.
...
- **Caso D (Payload viejo LS, nuevo IDB, sin puntero actualizado):** Imposible lógicamente. El puntero es el *Commit L2*. Hasta que el puntero no sobreescribe la key en LS, el sistema considera válida la versión de LS.
...
- **Arquitectura Mínima:** La clonación de los *elementos visuales* (la carpeta en el canvas) y la actualización del *BoardsGraph* ocurren síncronamente. El usuario ve la nueva carpeta inmediatamente.
- La clonación del *BoardData* (profundo) se despacha en *background* (Promesa).
...
### 7. VERSIONADO
**No se requiere un revision ID granular por board.**
El `BoardsGraph` usa *Last-Write-Wins* estricto y actúa como fuente de verdad jerárquica. Las escrituras de tableros individuales son atómicas y apuntan a un solo destino (el `boardId` UUID). La colisión de dos pestañas editando *el mismo board* se resuelve con Last-Write-Wins físico. Un reloj divergente no corrompe la persistencia porque las claves de versión simplemente notifican cambio, no imponen un orden causal distribuido (no somos un CRDT).
...
- *Archivos:* `LocalStorageBoardRepository.ts`
- *Cambio:* Añadir la lógica `idb-keyval`, el puntero `{__idb_pointer:true}` y los bloques `try/catch` de `QuotaExceededError`.
- *Prueba:* Llenar LS intencionalmente y comprobar que el board guarda en IDB.

**FASE 10B: GC y Ciclo de Vida**
- *Archivos:* `LocalStorageBoardRepository.ts`
- *Cambio:* Implementar `active-writes` (WAR) y la rutina `runGarbageCollector(graph)`. Actualizar la lógica de `clonePhysicalBoards` para ser async safe.
- *Prueba:* Creación rápida vs GC simulado.

**FASE 10C: Multi-Tab Signaling**
- *Archivos:* `app_constants.ts`, `boardService.ts`
- *Cambio:* Definir la key de versión. Añadir `window.addEventListener("storage")` para recargar el `BoardsGraph` en Jotai y hacer fallback a raíz si el currentFolder se borra.
- *Prueba:* Abrir dos ventanas y eliminar una carpeta en A; B debe cerrarla.
...
with open("arquitectura_fase_10_definitiva.md", "w", encoding="utf-8") as f:
    f.write(report)
